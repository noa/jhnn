...
static int jhu_THSample(lua_State *L) {
...
#pragma omp parallel for private(t, d, input_data, output_data)
  for (t = 0; t < nframe; ++t) {
    ...
    for (d=0; d<dim-1; ++d) {
      ...
      input_data[d+1] += input_data[d];
    }

    /* sample a probability mass from a uniform distribution */
    double uniform_sample = THRandom_uniform(_generator, 0, 1) * input_data[dim-1];
...
static const struct luaL_Reg jhu_THSample__ [] = {
  {"sample", jhu_THSample},
  {0, 0}
};

static void jhu_THSample_init(lua_State *L) {
  int ret = luaT_pushmetatable(L, "torch.DoubleTensor");
  if(ret == 0) {
    THError("problem pushing metatable");
  }
  luaT_registeratname(L, jhu_THSample__, "jhu");
  lua_pop(L, 1);
}
